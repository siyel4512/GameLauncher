...
public class FilePath : MonoBehaviour
{
    ...
    public void CompareToFileDownloadURL(int buttonNum)
    {
        ...
        for (int i = 0; i < buildFileUrls.Length; i++)
        {
            ...
            {
                Debug.Log($"[Compare] {i} 경로 변경됨");
                Debug.Log($"[Compare] {i} : %%{exeFolderPaths[i]}$$");
                if (exeFolderPaths[i] == "")
                {
                    Debug.Log($"[Compare] {i} : {exeFolderPaths[i]}");
                    // 임시 저장
